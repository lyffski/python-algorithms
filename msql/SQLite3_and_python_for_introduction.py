...
def print_db():
    ...
    # retrieve from
    try: # YOU ALWAYS YOU TRY BLOCK, WHEN YOU ARE WORKING WITH DATABASE, cuz u might have issues
        result = the_cursor.execute("SELECT id, f_name, l_name, age, address, salary, hire_date FROM employees") #cursor, travelse through the tables of db
        # u use exectue all and over agian, it just how python intreact with sqlite3
...
# db_conn.commit() // to run the execute func, they are linked
try:
...
try:
    db_conn.execute("ALTER TABLE employees ADD COLUMN 'image' BLOB DEFAULT NULL")  # Alter, let you modify the already existing table, according to you wish
    db_conn.commit()
except sqlite3.OperationalError:
...
# as a list
row_names = [nameTuple[1] for nameTuple in the_cursor.fetchall()] # list comphrisemieng,
print(row_names)
...
# Write data to File
with open('dump.sql', 'w') as f: # for backup
